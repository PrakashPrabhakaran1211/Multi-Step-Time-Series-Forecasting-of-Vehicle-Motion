...
def MLP_model(xTrain, xTest, yTrain, yTest):
    ...
    model = Sequential()
    model.add(Dense(1000, activation='relu', input_shape=(n_input,)))
    ...
    history =model.fit(xTrain, yTrain, epochs=20, verbose=1, validation_data=(xTest, yTest))

    # save the trained model
    filepath = 'trained_models/MLP_model.h5'
    model.save(filepath)

    # load trained model
    model = keras.models.load_model('trained_models/MLP_model.h5')
    ...
    plt.savefig('loss_visualization/loss_comparison_MLP.pdf')
