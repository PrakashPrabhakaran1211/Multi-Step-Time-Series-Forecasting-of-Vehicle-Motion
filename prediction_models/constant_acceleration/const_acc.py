...
def my_constant_acc_model(test_data, pred_horizon, samp_time, frame_range):
    prediction = list()
...
def my_prediction(predData, currFrame, trackID, predHorizon, samplingTime):
    ...
    xAccInit = float(predData.loc[(predData['frame'] == currFrame) &
                                  (predData['trackId'] == trackID), 'xAcceleration'])
    ...
    #x= ut +(at**2 /2)
    b = np.identity(predHorizon) * ((xVelInit * samplingTime) + (xAccInit * (samplingTime**2)/2))
    xCenter = list(x0 + np.matmul(a, b))
    # yCenter prediction
    yVelInit = float(predData.loc[(predData['frame'] == currFrame) &
                                  (predData['trackId'] == trackID), 'yVelocity'])
    yAccInit = float(predData.loc[(predData['frame'] == currFrame) &
                                  (predData['trackId'] == trackID), 'yAcceleration'])
    ...
    b = np.identity(predHorizon) * ((yVelInit * samplingTime) + (yAccInit * (samplingTime**2)/2))
    yCenter = list(y0 + np.matmul(a, b))

    prediction = xCenter + yCenter
    print(prediction[1])
    return prediction
